...
function loadDB(){
if(!fs.existsSync(DB_FILE))return{missions:[],soustraitants:[],personnel:[],users:[{id:'u1',nom:'Haimane',role:'Administrateur',pin:'0000',isAdmin:true},{id:'u2',nom:'Shanaz',role:'Comptabilité',pin:'1111'},{id:'u3',nom:'Fatima Zara',role:'Paiements',pin:'2222'},{id:'u4',nom:'Miryem',role:'Coordination',pin:'3333'}],config:{accessCode:'AMSBAT2026',nextMis:1,nextItv:1}};
try{return JSON.parse(fs.readFileSync(DB_FILE,'utf8'));}catch(e){return{missions:[],soustraitants:[],personnel:[],users:[],config:{accessCode:'AMSBAT2026',nextMis:1,nextItv:1}};}
}
...
socket.on('equipe:add',({type,data})=>{const d=loadDB();if(type==='st'){if(!d.soustraitants)d.soustraitants=[];d.soustraitants.push(data);}else if(type==='per'){if(!d.personnel)d.personnel=[];d.personnel.push(data);}saveDB(d);io.emit('update',d);});
